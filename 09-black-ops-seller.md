...
> **Summary.** "Black Ops" is an elite, rapid-deployment **operating mode** within the Field CTO group: a small special-forces "tiger team" that parachutes into the hardest, highest-value, most strategically important accounts to **break them open** — then hands the account back to the standard Field CTO + account team for sustained land-and-expand. The mission is deliberately focused, but the point is **durable growth for Cisco**: a Black Ops breakthrough is the *ignition* of a lasting, compounding, multi-year relationship — not a one-off engagement that ends when the pod leaves. It is a concentrated breakthrough capability layered on top of the Field CTO *marathon* of sustained coverage.
>
> **Name disclaimer:** "Black Ops" denotes the **special-forces posture only** — elite, cross-functional, mission-scoped, and outcome-obsessed. It is **fully ethical and above-board**. There are no grey-area tactics, nothing hidden from the customer, and every OT-safety, read-only, and human-in-the-loop guardrail still applies without exception.
...
## 2. Marathon vs. breakthrough — how Black Ops differs from the standard Field CTO

| Dimension | Field CTO (standard) | Black Ops (tiger team) |
|-----------|----------------------|-------------------------|
| **Motion** | Sustained portfolio coverage (marathon) | Mission-based breakthrough on one target |
| **Scope** | 5–8 named accounts | One hard target at a time |
| **Duration** | Ongoing | Focused ~60–120-day mission that **ignites durable growth** |
...
The critical design principle: **Black Ops does not keep the account.** Its job is to break it open and hand it back, so the sustained team can close and grow it into a durable, expanding Cisco relationship. This prevents key-person dependency and keeps the tiger team available for the next hard target.
...
|-------|----------------|--------------|--------|
| **1. Mission charter** | 0–5 | Chief Field CTO scopes the target, objective, pod, and mission window | Charter + success definition |
...
| Component | Basis |
|-----------|-------|
| **Mission success (primary)** | Breakthrough achieved: executive access created + X-Arch architecture framed + qualified pipeline generated during the mission |
| **Durable growth (the real prize)** | Additive credit on bookings from the account after hand-back — the lasting, compounding, multi-year growth the mission was launched to create (shared with the sustained team) |
...
> The Field CTO group runs the **marathon** of sustained coverage; Black Ops is the **breakthrough** — an elite tiger team that cracks open the accounts nobody else can and **ignites durable, multi-year growth for Cisco**, then hands them back to be closed and grown.
